...
class DoscotiabankPipeline:

    # Database setup
    conn = sqlite3.connect('doscotiabank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `doscotiabank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM doscotiabank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO doscotiabank (date, title, link, content)"
